...
`VITE_SITE_URL` overrides the published URL baked into social cards and the share dialogs (default `https://github.com/Ppratik765/Offline-file-transfer-app/`, trailing slash required).
...
- **`release.yml`** — on a `v*` tag: builds everything, attaches `wavedrop-<tag>-site.zip`, both standalone files, and `SHA256SUMS.txt`.
