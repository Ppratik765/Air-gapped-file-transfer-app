// Regenerates the PWA icon set in public/ from public/wavedrop_logo.svg:
//
...
const logo = readFileSync(join(publicDir, "wavedrop_logo.svg"), "utf8");
...
function squareVariant(markScale: number): string {
  for (const needle of [ROUNDED_RECT, MARK_OPEN]) {
    if (!cleaned.includes(needle)) {
      throw new Error(`wavedrop_logo.svg changed shape — expected to find: ${needle}`);
    }
  }
  return cleaned
...
const work = mkdtempSync(join(tmpdir(), "wavedrop-icons-"));
