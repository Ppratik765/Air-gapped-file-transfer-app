...
export function initAppPromoBanner(): void {
  ...
  const apkBtn = document.getElementById("download-apk-btn") as HTMLAnchorElement;
  ...
  if (isInsideNativeApp()) {
      ...
      .then(data => {
        if (data && data.version && compareVersions(data.version, currentVersion) > 0) {
          ...
          if (apkBtn) {
            ...
            apkBtn.href = "https://ppratik765.github.io/Offline-file-transfer-app/wavedrop.apk"; 
    ...
    if (apkBtn) {
      // Set relative URL for web app users downloading the APK
      apkBtn.href = "../wavedrop.apk";
      apkBtn.addEventListener("click", () => {
        localStorage.setItem("wavedrop_downloaded_apk_version", currentVersion);
        // Hide the banner immediately after click
        banner.style.display = "none";
      });
    }
